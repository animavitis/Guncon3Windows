...
{
    /// <summary>
    /// Fits and applies a projective transform between two quadrilaterals.
    /// </summary>
    public static class Homography
    {
        ...
        {
            if (src.Length != 4 || dst.Length != 4) throw new ArgumentException("Need 4 correspondences (the corners).");
        ...
        private static double[] SolveLinear8(double[,] A, double[] b)
        {
            ...
            for(int col=0; col<n; col++)
            {
                ...
                if (Math.Abs(M[piv,col])<1e-12) throw new InvalidOperationException("Singular system: the correspondences are degenerate.");
